 ...
 */
class HttpClient(
    private val configuration: HttpClientConfiguration = HttpClientConfiguration.builder().build()
) {
     ...
     * @param onResult A callback that receives the [Result] of the request, containing either a [Response]
     ...
     * on success or an exception on failure.
     */
    fun executeHeadRequest(request: HeadRequest, onResult: (Result<Response>) -> Unit) {
        executeRequest(request, onResult)
    }
     ...
     * on success or an exception on failure.
     */
    fun executeOptionsRequest(request: OptionsRequest, onResult: (Result<Response>) -> Unit) {
        executeRequest(request, onResult)
    }
     ...
     * on success or an exception on failure.
     */
    fun executePatchRequest(request: PatchRequest, onResult: (Result<Response>) -> Unit) {
        executeRequest(request, onResult)
    }
     ...
     * on success or an exception on failure.
     */
    private fun executeRequest(request: Request, onResult: (Result<Response>) -> Unit) {
            ...
            connection.run {
                ...
                if (isRequestSuccessful(responseCode)) {
                    ...
                    if (location != null) {
                        ...
                        executeRequest(redirectRequest, onResult) // Follow redirect
                    } else {
                        throw RedirectException(request.url, responseCode, "No Location header")
                    }
                } else {
     ...
     */
    private fun isRequestSuccessful(responseCode: Int): Boolean =
