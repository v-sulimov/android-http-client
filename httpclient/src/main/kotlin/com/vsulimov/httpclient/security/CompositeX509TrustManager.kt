...
class CompositeX509TrustManager : X509TrustManager {
    ...
    /** The list of [X509TrustManager] instances used for certificate validation. */
    ...
    constructor(keyStores: List<KeyStore>) {
        trustManagers = listOf(getDefaultTrustManager()) + keyStores.map { getTrustManagerForKeyStore(it) }
    }
    ...
    /**
     * Returns the combined list of accepted issuers from all trust managers, without duplicates.
     *
     * Deduplication is performed by subject distinguished name (case-insensitive), which matches
     * how certificate equality is evaluated in practice.
     *
    ...
    override fun getAcceptedIssuers(): Array<X509Certificate> =
        trustManagers.flatMap { it.acceptedIssuers.toList() }
