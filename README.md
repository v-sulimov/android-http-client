...
- **Streaming I/O**: Stream large downloads or uploads without buffering the entire payload in memory.
...
For large downloads or uploads, use the streaming variants to avoid buffering the entire payload in memory.

**Streaming download (GET):**
...
val result = client.executeStreamingPostRequest(request) { response ->
    println("Upload response: ${response.statusCode}")
}
result.onFailure { exception ->
...
The `streamHandler` lambda receives a `StreamingResponse` containing the status code, headers, and a raw `InputStream`. The connection is kept open while the lambda executes and is disconnected immediately after it returns — do not retain a reference to the body stream beyond the scope of the handler.

Streaming methods are available for GET (`executeStreamingGetRequest`), POST (`executeStreamingPostRequest`), PUT (`executeStreamingPutRequest`), and PATCH (`executeStreamingPatchRequest`).
